...
# ---------------------------------------------------------------------------
# 让位：VBE 自带的「自动列出成员」下拉列表
# ---------------------------------------------------------------------------
#
# VBE 的自动完成列表跟我们的弹窗会出现在同一处（光标正下方）。两个列表同时
# 出现会互相遮挡、还抢键盘，所以 VBE 一旦自己弹了列表，我们就【不弹】。
#
# 探测全走 Win32、不碰 COM。三条互相独立的判据，任一命中即算「VBE 列表已
# 出现」——之所以要三条，是因为这个列表窗口的类名与父子关系随 VBE 版本、
# 宿主而变，任何单条判据都可能在某些机器上整条失效：
#   A 已知类名 —— 命中即算（可用 VBECOMPLETE_VBE_LIST_CLASS 追加）；
#   B 属主弹窗 —— VBE 主窗口 OWNED 的、可见的、无标题栏的顶层弹窗；
#                 下拉列表正是这个形态，而浮动工具窗 / 对话框都有标题栏；
#   C 光标下方 —— 光标正下方那点被【VBE 进程的无标题栏浮动弹窗】盖住。
#                 与类名、父子关系全都无关，是最通用的一道兜底。
#
# 拿不准时一律【不拦】（返回 False）：宁可两个列表同时出现，也不能让工具
# 整个哑掉。可用 VBECOMPLETE_NO_YIELD=1 关掉这套让位行为。

_YIELD_TO_VBE_LIST = (
    os.environ.get("VBECOMPLETE_NO_YIELD", "0").strip() != "1")

_VBE_LIST_CLASSES_BASE = ("VBEListBox", "VbaListBox", "VbxCCListBox",
                          "CCListBox")
_extra_cls = os.environ.get("VBECOMPLETE_VBE_LIST_CLASS", "")
_VBE_LIST_CLASSES = tuple(
    list(_VBE_LIST_CLASSES_BASE)
    + [c.strip() for c in _extra_cls.split(",") if c.strip()])
...
def _is_bare_popup(h):
    """是否「可见的、无标题栏的顶层弹窗」——下拉列表 / 浮动提示的典型形态。

    关键在【无标题栏】：浮动工具窗、对话框、消息框都带 WS_CAPTION，会被排除。
    否则只要「工程资源管理器」浮在别处，补全就永远弹不出来了。
    """
    try:
        import ctypes
        ...
        if not u32.IsWindowVisible(h):
            return False
        st = u32.GetWindowLongW(h, _GWL_STYLE) & 0xFFFFFFFF
        return bool(st & _WS_POPUP) and not (st & _WS_CAPTION)
    except Exception:
        return False


def _looks_like_list(w, h):
    """尺寸是否像一个下拉列表：太小的工具条、太大的窗口都不算。"""
    return 120 <= w <= 1400 and 24 <= h <= 600
...
def _vbe_main_hwnd():
    ...
    try:
        ...
        return int(u32.FindWindowW("wndclass_desked_gsk", None) or 0)
    except Exception:
        return 0


def _find_list_by_class(main_h):
    """判据 A：按已知类名找 VBE 的列表窗口。"""
    try:
        import ctypes
...
def _find_list_owned(main_h):
    """判据 B：VBE 主窗口 OWNED 的、可见的、无标题栏的顶层弹窗。"""
    try:
        import ctypes

        u32 = ctypes.windll.user32
        h = u32.GetWindow(main_h, _GW_OWNED)
        n = 0
        while h and n < 64:                      # 上限防死循环
            if _is_bare_popup(h):
                w, ht = _wnd_size(h)
                if _looks_like_list(w, ht):
                    return int(h)
            h = u32.GetWindow(h, _GW_OWNED)
            n += 1
    except Exception:
        pass
    return 0
...
def _list_at_caret(caret_rect, main_h):
    """判据 C：光标正下方那点被【VBE 进程的无标题栏浮动弹窗】盖住。

    caret_rect 是 ui.caret_screen_rect() 的返回值 (x, top, bottom)。
    VBE 的列表就从光标下面那一行开始画，所以探这一点最准；命中窗口必须是
    VBE 进程的（我们自己的弹窗属于别的进程，天然被排除）。
    """
    if not caret_rect:
        return 0
    try:
        ...
        x, top, bottom = caret_rect
        u32 = ctypes.windll.user32
        vbe_pid = _wnd_pid(main_h)
        if not vbe_pid:
            return 0
        pt = wintypes.POINT(int(x),
                            int(bottom + max(4, (bottom - top) // 2)))
        h = u32.WindowFromPoint(pt)
        if not h:
            return 0
        if _wnd_pid(h) != vbe_pid:
            return 0        # 是我们自己的弹窗 / 别的程序 -> 不算
        if not _is_bare_popup(h):
            return 0
        return int(h)
    except Exception:
        return 0
...
_vbe_list_logged = set()


def _note_vbe_list(h, how):
    """把命中的窗口记一行日志（每种签名只记一次），便于日后校对准星。

    这个列表窗口的形态在不同机器上未必一样，留一条日志，日后只要翻日志就
    知道机器上真正的类名 / 尺寸，不用再抓瞎。
    """
    try:
        sig = (how, _wnd_class(h)) + tuple(_wnd_size(h))
        if sig in _vbe_list_logged:
            return
        _vbe_list_logged.add(sig)
        _log("vbe-list: %s 命中 hwnd=%s class=%r size=%s"
             % (how, h, _wnd_class(h), _wnd_size(h)))
    except Exception:
        pass


def vbe_list_visible(caret_rect=None):
    """VBE 自己是不是正在显示「自动列出成员」列表（是则我们让位、不弹）。

    caret_rect 为 ui.caret_screen_rect() 的返回值，用于判据 C；拿不到就只
    走 A / B（照样能工作）。拿不准 / 出任何异常一律返回 False（不拦）。
    """
    if not _YIELD_TO_VBE_LIST:
        return False
    try:
        main_h = _vbe_main_hwnd()
        if not main_h:
            return False                  # VBE 没开 -> 无从判断 -> 不拦
        for how, finder in (("A-class", _find_list_by_class),
                            ("B-owned", _find_list_owned)):
            h = finder(main_h)
            if h:
                _note_vbe_list(h, how)
                return True
        h = _list_at_caret(caret_rect, main_h)
        if h:
            _note_vbe_list(h, "C-caret")
            return True
        return False
    except Exception:
        return False
